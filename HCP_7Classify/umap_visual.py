...
if __name__ == "__main__":
    ...
    TSNE_SAVE_PATH = os.path.join(LOG_DIR_PATH, "umap_10_fine_tuning.png")
    ...
    print(f"特征提取完毕。最终用于umap的样本数: {all_features.shape[0]}")

    # --- 运行 UMAP 并可视化 ---
    print("运行 UMAP 降维...")

    # UMAP的参数:
    # n_neighbors: 每个点考虑的邻居数量。值越小，越关注局部结构；值越大，越关注全局结构。可以从15开始尝试。
    # min_dist: 控制嵌入空间中点的紧凑程度。值越小，簇越紧密；值越大，簇越分散。可以从0.1开始。
    # metric: 距离度量，'euclidean' 或 'cosine' 都是不错的选择。
    umap_reducer = UMAP(
        n_neighbors=20,
        min_dist=0.2,
        n_components=2,
    ...
    umap_results = umap_reducer.fit_transform(all_features)  # all_features是(样本数, 特征维度)的Numpy数组
    print("UMAP 运行完毕。")

    df = pd.DataFrame(
        {'umap-1': umap_results[:, 0], 'umap-2': umap_results[:, 1], 'label': [tasks[i] for i in all_labels]})
    ...
    plt.title('Umap of Aggregated Scan-Level Features', fontsize=20)
