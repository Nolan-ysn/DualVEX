...
class ClassifyConfig:
    ...
    # PEFT 库会调用 model.config.get("tie_word_embeddings") 来检查配置
    # 我们添加这个方法，让 ClassifyConfig 类支持 .get() 调用
    @classmethod
...
def main():
    ...
    log_name = f"HCP_7Class_MAE_ST_{ClassifyConfig.num_train_subjects}_LoRA_{date_str}"
    ...
    model = EncoderClassifier(ClassifyConfig)
    ...
    if ClassifyConfig.use_lora:
        logger.info("Applying LoRA to the model...")
        peft_config = LoraConfig(
            inference_mode=False,
            r=ClassifyConfig.lora_r,
            lora_alpha=ClassifyConfig.lora_alpha,
            lora_dropout=ClassifyConfig.lora_dropout,
    ...
    # 4. [修改] 优化器
    # 因为 LoRA 已经把不需要梯度的参数冻结了，我们只需要把 trainable params 传给优化器
    logger.info(f"Optimizer: Updating only trainable parameters. LR: {ClassifyConfig.learning_rate}")
    ...
    # 5. [修改] 学习率调度器
    # 既然只有一组参数，直接用 CosineAnnealingLR
