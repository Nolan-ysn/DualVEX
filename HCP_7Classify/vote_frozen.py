...
def main():
    ...
    log_name = f"HCP_7Class_MAE_ST_{ClassifyConfig.num_train_subjects}_frozen_{date_str}"
    ...
    # 冻结除 'head' 以外的所有参数
    logger.info("Freezing pretrained encoder parameters...")
    for name, param in model.named_parameters():
        if name.startswith('head'):
            param.requires_grad = True
            logger.info(f"Trainable: {name}")  # 打印确认分类头是可训练的
        else:
            param.requires_grad = False  # 冻结预训练部分 (Encoder, TimeEmbed, PosEmbed)

    # 只需要收集 Head 的参数给优化器
    # 注意：这里不再区分 backbone_params，因为它们不需要更新
    ...
    optimizer = torch.optim.AdamW(
        head_params,
        lr=ClassifyConfig.learning_rate_head,
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        ...
        eta_min=ClassifyConfig.eta_min_head
    )
